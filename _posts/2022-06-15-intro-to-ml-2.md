...
## Cross-Validation
**Cross-Validation** is a process of validating the model on different subsets of data. Validating on fixed sets of validation data can lead to inaccurate measure of model quality. It may perform well on given validation set, but it may not for other sets of data. **Cross-Validation** is used to prevent this. 


In **cross-validation**, dataset is divided into small subsets of equal size. Each subset are called **folds**. Then, we run one iteration for each fold. On each iteration each fold is set as a validation (holdout) and every other folds as training data. This gives us measure of model quality validated on entire dataset.

![cross-validation](https://i.imgur.com/qXV7dSk.png)

Tradeoff of **cross-validation** is longer running time. While it may insignificant for small dataset, cross-validating on larger datasets may take much longer time. It may also be unneccesary. On larger datasets, single validation set may be sufficient enough yielding same scores as cross-validation.
...
## Links
* [Intro to ML Course on Kaggle](https://www.kaggle.com/learn/intermediate-machine-learning)
